...
test('Login y desplegar Responsable', async t => {
    ...
    const toggleIcon = Selector('.a-TreeView-toggle').nth(0); // Primer ícono de expansión
    const subMenu = Selector('#t_TreeNav_1_subtree');
    const RespMenu = Selector('.a-TreeView-label').withText('Responsable');
    const responsablesHeader = Selector('h1.t-Breadcrumb-label').withText('Responsables');
...
// Espera a que los campos estén disponibles
    ...
    await t.expect(RespMenu.exists).ok({ timeout: 5000 });

    // Hace clic en el enlace "Responsables"
     await t.click(RespMenu);

     await t.expect(responsablesHeader.exists).ok({ timeout: 5000 });
});

test('Login y desplegar Proyectos', async t => {
    // Despliega el menú administración
    const adminMenu = Selector('#admin-menu');
    const submenu = Selector('.submenu');
    // Selecciona los campos usando el atributo 'name'
    ...
    const toggleIcon = Selector('.a-TreeView-toggle').nth(0); // Primer ícono de expansión
    const subMenu = Selector('#t_TreeNav_1_subtree');
    const ProyMenu = Selector('.a-TreeView-label').withText('Proyectos');
    const ProyectosHeader = Selector('h1.t-Breadcrumb-label').withText('Proyectos');
...
// Espera a que los campos estén disponibles
    ...
    // click en responsable
    await t.expect(ProyMenu.exists).ok({ timeout: 5000 });

    // Hace clic en el enlace "Responsables"
     await t.click(ProyMenu);

     await t.expect(ProyectosHeader.exists).ok({ timeout: 5000 });
});
...
test('Login y desplegar Solicitante', async t => {
    ...
    const toggleIcon = Selector('.a-TreeView-toggle').nth(0); // Primer ícono de expansión
    const subMenu = Selector('#t_TreeNav_1_subtree');
    const SoliMenu = Selector('.a-TreeView-label').withText('Solicitante');
    const SolicitanteHeader = Selector('h1.t-Breadcrumb-label').withText('Solicitante');
...
// Espera a que los campos estén disponibles
    ...
    // click en responsable
    await t.expect(SoliMenu.exists).ok({ timeout: 5000 });

    // Hace clic en el enlace "Responsables"
     await t.click(SoliMenu);

     await t.expect(SolicitanteHeader.exists).ok({ timeout: 5000 });
});