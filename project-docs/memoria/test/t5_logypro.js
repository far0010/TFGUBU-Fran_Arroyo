import {BUTTON, USERNAME, PASSW, TOGICON, SUBMEN, PROYECTMENU, PROYECTHEAD} from './constanst.js';
...
// Espera a que los campos estén disponibles
    ...
    await t.expect(PROYECTMENU.exists).ok({ timeout: 5000 });

    // Hace clic en el enlace "Responsables"
     await t.click(PROYECTMENU);

     await t.expect(PROYECTHEAD.exists).ok({ timeout: 5000 });
});