...
\imagen{listener}{Configuración Listener a FREEPDB1}
...
\imagen{services}{Servicios activados de Oracle}
...
\imagen{dirords}{Directorio de instalación ORDS}{.6}
...
\begin{itemize}
	...
	\item \textbf{dir databases}:
		\subitem{dir default}: Contiene la configuración por defecto, el subdirectorio \textbf{images} que contiene las imágenes de \acrfull{APEX} y el fichero de configuración de la conexión a la BD \textbf{pool.xml}		
\end{itemize}
...
\imagen{pool}{Fichero de configuración pool.xml}
...
Una vez instalados, deberemos proceder a la configuración de cada plug-in para su correcto funcionamiento, en \textbf{Shared Components/Component Settings} escogeremos cada plug-in y e introduciremos la web (en este caso al ser local http) y la \textbf{API key}, que nos identifica en el servicio y el modo de AOP, en este caso ya en Producción para que no inserte marca de agua.
...
\imagen{accioninforme}{Asociación de la plantilla al informe}
...
Una vez conseguido este propósito, procederíamos a crear nuestra instancia de la base de datos como \textbf{Base de datos autónoma} ~\cite:
